...
function doGet(e) {
  ...
  // データパース処理（Base64または生JSON）
  let dataObj = null;
  try {
    if (params.data64) {
      const decoded = Utilities.newBlob(Utilities.base64Decode(params.data64)).getDataAsString();
      dataObj = JSON.parse(decoded);
    } else if (params.data) {
      dataObj = JSON.parse(params.data);
    }
  } catch (err) {
    console.error("Data parse error:", err.toString());
  }

  try {
    switch (action) {
      case 'ping': return createJsonResponse({ success: true, message: 'pong', version: '20260201-2045' });
      ...
      case 'getSummaryCounts': return createJsonResponse(getSummaryCountsEnhanced());
      
      // 更新系アクション（エイリアス対応）
      case 'add':
      case 'memberAdd':
        return createJsonResponse(addMember(dataObj));
      case 'update':
      case 'memberUpdate':
        return createJsonResponse(updateMember(params.id, dataObj));
      case 'approveMember':
        return createJsonResponse(approveMember(params.id));
      case 'delete':
      case 'memberDelete':
        return createJsonResponse(deleteMember(params.id));
      
      case 'setupProfile':
        return createJsonResponse(setupProfile(params.email, dataObj));
      
      case 'addNews':
      case 'newsAdd':
        const nData = dataObj || (params.newsData ? JSON.parse(params.newsData) : null);
        return createJsonResponse(addNews(nData));
      case 'updateNews':
      case 'newsUpdate':
        const unData = dataObj || (params.newsData ? JSON.parse(params.newsData) : null);
        return createJsonResponse(updateNews(params.id, unData));
      case 'deleteNews':
      case 'newsDelete':
        return createJsonResponse(deleteNews(params.id));
      
      case 'addInternalAnnouncement':
      case 'announcementAdd':
        const aData = dataObj || (params.announcementData ? JSON.parse(params.announcementData) : null);
        return createJsonResponse(addInternalAnnouncement(aData));
      case 'updateInternalAnnouncement':
      case 'announcementUpdate':
        const uaData = dataObj || (params.announcementData ? JSON.parse(params.announcementData) : null);
        return createJsonResponse(updateInternalAnnouncement(params.id, uaData));
      case 'deleteInternalAnnouncement':
      case 'announcementDelete':
        return createJsonResponse(deleteInternalAnnouncement(params.id));

      case 'getInternalAnnouncements':
      case 'announcementsGet':
        return createJsonResponse({ success: true, data: getInternalAnnouncements() });
      ...
      case 'submitInquiry': return createJsonResponse(submitInquiryWithNotification(dataObj || (params.data ? JSON.parse(params.data) : null)));
      case 'debugInquiry': return createJsonResponse(debugInquiry());
      case 'getDebugInfo': return createJsonResponse(getDebugInfo());
      case 'getSheets': return createJsonResponse(getSheetsInfo());
      case 'verifyLineToken': return createJsonResponse(verifyLineToken(params.token));
...
function doPost(e) {
  ...
  try {
    if (e.postData && e.postData.contents) {
      postDataParams = JSON.parse(e.postData.contents);
    }
  } catch (err) {
    console.warn("JSON parse warning:", err.toString());
  }
  
  // Merge query parameters and post data (Body takes priority)
  const mergedParams = { ...e.parameter, ...postDataParams };
  const action = mergedParams.action;
  
  console.log("[doPost] Processing action:", action);
  
  try {
    switch (action) {
      // 会員管理
      case 'getMembers': return createJsonResponse({ success: true, data: getMembers() });
      case 'add': return createJsonResponse(addMember(mergedParams.data));
      case 'update': return createJsonResponse(updateMember(mergedParams.id, mergedParams.data));
      case 'approveMember': return createJsonResponse(approveMember(mergedParams.id));
      case 'delete': return createJsonResponse(deleteMember(mergedParams.id, mergedParams.data));
      case 'getInquiries': return createJsonResponse(getInquiries());
      case 'replyToInquiry': return createJsonResponse(replyToInquiry(mergedParams.id, mergedParams.email, mergedParams.message));
      case 'deleteInquiry': return createJsonResponse(deleteInquiry(mergedParams.id));
      case 'setupProfile': return createJsonResponse(setupProfile(mergedParams.email, mergedParams.data));
      
      // ニュース管理
      case 'getNews': return createJsonResponse({ success: true, data: getNews() });
      case 'addNews': return createJsonResponse(addNews(mergedParams.newsData));
      case 'updateNews': return createJsonResponse(updateNews(mergedParams.id, mergedParams.newsData));
      case 'deleteNews': return createJsonResponse(deleteNews(mergedParams.id));
      
      // その他
      case 'updateAttendance': return createJsonResponse(updateAttendance(mergedParams));
      case 'updateAccounting': return createJsonResponse(updateAccounting(mergedParams));
      case 'uploadFile': return createJsonResponse(uploadFile(mergedParams));
      case 'submitInquiry': return createJsonResponse(submitInquiry(mergedParams.data));
      
      // 部員向けお知らせ
      case 'getInternalAnnouncements': return createJsonResponse({ success: true, data: getInternalAnnouncements() });
      case 'addInternalAnnouncement': return createJsonResponse(addInternalAnnouncement(mergedParams.announcementData));
      case 'updateInternalAnnouncement': 
        return createJsonResponse(updateInternalAnnouncement(mergedParams.id, mergedParams.announcementData));
      case 'deleteInternalAnnouncement': 
        return createJsonResponse(deleteInternalAnnouncement(mergedParams.id));
      
      default:
        console.warn("[doPost] Unknown action requested:", action);
        return createJsonResponse({ success: false, error: "Unknown action: " + action });
    }
  } catch (err) {
    console.error("[doPost] Fatal error:", err.toString());
    return createJsonResponse({ success: false, error: err.toString() });
  }
}
...
/**
 * 学年表記をスプレッドシートの入力規則（小1, 中2等）に変換する
 */
function normalizeGradeValue(value) {
  ...
  // 正規表現での部分一致変換
  if (v.match(/^小学([1-6])年?$/)) return '小' + v.match(/^小学([1-6])年?$/)[1];
  if (v.match(/^中学([1-3])年?$/)) return '中' + v.match(/^中学([1-3])年?$/)[1];
  if (v.match(/^高校([1-3])年?$/)) return '高' + v.match(/^高校([1-3])年?$/)[1];
...
function addMember(data) {
  try {
    ...
    if (adjustedData.grade) {
      adjustedData.grade = normalizeGradeValue(adjustedData.grade);
    ...
    console.error("[addMember] Error:", err.toString());
...
function getNews() {
  ...
  const sheet = getSheetAllowAliases(ss, NEWS_SHEET_NAME);
  if (!sheet) return [];
 ...
 * 部員向けお知らせを更新
 */
function updateInternalAnnouncement(id, data) {
  try {
    const ss = getSS(MEMBER_SPREADSHEET_ID);
    const sheet = ss.getSheetByName(INTERNAL_ANNOUNCEMENTS_SHEET_NAME);
    if (!sheet) return { success: false, error: 'Sheet not found' };
      ...
      if (String(values[i][0]).trim() === targetId) {
        ...
        headers.forEach((h, colIdx) => {
          if (updateMap[h] !== undefined) {
            sheet.getRange(rowNumber, colIdx + 1).setValue(updateMap[h]);
          }
        });
...
function getDebugInfo() {
  const info = { version: "20260201-2045", timestamp: new Date().toISOString(), connected: {}, memberHeaders: [] };
  try {
    const ss = getSS(MEMBER_SPREADSHEET_ID);
    info.connected.member = "OK";
    const sheet = getSheetAllowAliases(ss, MEMBER_SHEET_NAME);
    if (sheet) {
      info.memberHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      info.headerMapping = info.memberHeaders.map(h => ({ header: h, key: normalizeKey(h) }));
    }
...
function normalizeKey(header) {
  ...
  if (h.includes('権限') || h.includes('role') || h.includes('管理') || h.includes('admin')) return 'role';
  if (h.includes('備考') || h.includes('メモ') || h.includes('notes')) return 'notes';

  // 会員向け
  if (h.includes('line') && h.includes('id')) return 'lineId';
  if (h.includes('世帯') || h.includes('family')) return 'familyId';
...
/**
 * LINEアクセストークンの検証 (簡易版)
 * プロフィール情報を取得して、登録されているメールアドレスと照合
 */
function verifyLineToken(token) {
  ...
  try {
    ...
    if (!profile.userId) throw new Error("Invalid LINE profile");
    ...
    return { success: false, error: "LINE検証エラー: " + e.toString() };
  }
}
